...
next PATCH mutates the shared file (GPT round-13 finding F1). Writes must be
...
class TestAgentStateSidecarWriteProtection:
    ...
    def test_bash_write_is_sealed_by_the_sandbox(self) -> None:
        ...
        # parsing: main's security refactor (#9089/#9183) moved the path fence
        # to the layer that can hold it, so the sidecar must sit in BOTH seal
        ...
        # The lock sibling too (GPT round-38): an unsealed lock can be
    ...
    def test_template_spec_lock_is_sealed_by_the_sandbox(self) -> None:
        # GPT round-39: agents_spec_lock's .kirocrew-agents.lock lives in the
        # KIRO AGENTS tree, not the crew home, so it rides its own readonly
        # target and its own pre-create entry (resolved via kiro_agents_dir).
        from kiro_crew import sandbox

        assert ".kiro/agents/.kirocrew-agents.lock" in sandbox._CREW_READONLY_TARGETS
        _dirs, files = sandbox._sealable_absent_ceilings()
        assert any(f.endswith(".kirocrew-agents.lock") for f in files), files
