...
def git(monkeypatch):
    ...
    # `_direct_push`'s HEAD-identity gate (#8452) resolves TWO revisions with `rev-list -1`
    ...
    # And the retry proves the rebase replayed exactly ONE commit (#8452); the longer
    ...
    # than from the mutable `FETCH_HEAD` ref (#8452), so the fetch has to report one; an
...
def test_direct_push_refuses_an_unreadable_pushable_diff(tmp_path, git):
    ...
    # The scan reads the COMMITTED OBJECT BY ID (#8452), not `HEAD` -- binding it to the
...
def test_direct_push_records_a_failed_push(tmp_path, git):
    ...
    # The recorded sha is the OBJECT THAT WAS SENT (#8452), which the shared `git` fixture
...
def test_direct_push_reports_the_sha_that_actually_landed(tmp_path, git):
    ...
    It used to be a post-push `rev-parse HEAD`, which names whatever HEAD points at by then
    -- so a concurrent move put an unrelated commit in the ledger for a change that really
    did land. `_push_with_rebase` now reports the revision it sent, and this asserts the
    ledger follows that rather than the ref (#8452). `rev-parse HEAD` is deliberately
    scripted to a DIFFERENT value here, so a regression to the old behaviour reddens.
    """
    git.script("rev-parse --verify", 0)
    git.script("diff --no-ext-diff committedsha~1..committedsha", (0, DIFF, ""))
...
def test_direct_push_reporting_survives_a_blank_rev_parse(tmp_path, git):
    """REPLACES a test that pinned falling back to the caller's snapshot when
    `rev-parse HEAD` blanked. That fallback is no longer reachable on this path, because the
    recorded sha comes from the object that was sent rather than from a post-push ref read --
    which is a stronger version of the same guarantee the old test wanted ("a reporting
    hiccup cannot blank a real sha"). The fallback itself remains for callers that do not
...
def test_direct_push_reads_the_fetch_url_off_the_clone_when_the_profile_has_none(tmp_path, git):
    ...
    # The refspec source is the OBJECT ID the gate retained (#8452), not `HEAD`: an id cannot
...
def test_fetched_tip_oid_only_accepts_a_real_object_id(stdout, expect):
    ...
    mutable file the pre-push reviewer can repoint at a commit nothing scanned, and both the
    rebase and the replayed-commit count used to read it. Anything this cannot parse must come
...
def test_reset_provisional_rolls_the_branch_back_atomically(tmp_path, git):
    ...
    three rounds of the GPT review of #8452."""
    d = _make(tmp_path)
...
def test_a_failed_rollback_quarantines_the_clone_so_a_later_run_cannot_adopt_it(tmp_path, git):
    ...
    that is exactly why the protection cannot live there. Raised by the GPT review of #8452;
...
def test_a_second_rollback_after_quarantine_is_a_no_op_and_does_not_mislead(tmp_path, git, caplog):
    ...
    check out a path that no longer exists, fail, and then report "left unsafe in place" -- which
...
def test_a_failed_rollback_halts_the_run_and_says_why(tmp_path, git, caplog):
    ...
    rollback IS the repair, so the run stops. Raised by the GPT review of #8452."""
    git.script("checkout -f -B", (1, "", "cannot checkout"))
    d = _make(tmp_path)
    with caplog.at_level(logging.ERROR, logger=LOG.name):
