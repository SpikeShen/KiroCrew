...
class TestLoadSecurityPolicyPrecedence:
    def test_the_central_source_outranks_the_local_env_file(
        ...
        # Fetched, not short-circuited: the env file no longer stops the central tier
...
class TestTheSignatureGateReadsTheSameStrictFlagAsTheConstructor:
    ...
    and fails CLOSED. The gate that ENFORCES the flag used to re-open the file and call
    ``bool()`` on the raw value, so at both enforcement points (boot, and the fetch
    refusal in ``policy_distribution``) that same ``null`` read the gate OFF and an
