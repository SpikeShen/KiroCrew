...
def test_every_production_call_site_uses_the_off_loop_form():
    ...
    # Two production sites, reached two different ways for the reason the
    # cancel-path test below spells out: the success path awaits it, the
    # cancel/error path dispatches it and adds no suspension point.
    assert _SRC.count("await _flush_file_changes_off_loop(slot)") == 1
    assert _SRC.count("asyncio.create_task(_flush_file_changes_off_loop(slot))") == 1
...
def test_the_cancel_path_never_awaits_the_flush():
    ...
    dispatch = "asyncio.create_task(_flush_file_changes_off_loop(slot))"
