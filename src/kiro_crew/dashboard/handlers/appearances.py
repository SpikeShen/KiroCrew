...
from kiro_crew.apps.builtins.crew_companion.appearances import DEFAULT_PACK
...
#: How long a browser may reuse a slot's or a cue's bytes. Short: a pack the
...
async def api_appearance_detail(request: web.Request) -> web.Response:
    ...
    if detail is None:
        # 404 rather than 400: an id that no longer resolves is the normal
...
async def api_appearance_sound(request: web.Request) -> web.Response:
    ...
    ``pack_detail``'s ``sounds`` map already says which states exist, so a client
    only reaches this route for a cue it knows is there.
    """
    ...
    found = await asyncio.to_thread(
        store.pack_sound, request.match_info["id"], request.match_info["state"]
    )
    if found is None:
...
async def api_appearance_delete(request: web.Request) -> web.Response:
    ...
    The guard itself is ``dashboard.appearances.delete_pack_if_unworn``, shared
    with Crew Companion's own gallery delete — both doors now reach the same
    library, so a rule on only one of them is not a rule.
    """
