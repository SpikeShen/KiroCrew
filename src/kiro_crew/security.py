...
_CREW_SECRET_LEAVES: list[str] = [
    ...
    # stages its temp file in here before renaming onto its target, so during a
    ...
    # present and future, is covered. A TOP-LEVEL leaf, not one under
    # ``workspace/md-notebook``, so an agent-writable ancestor cannot be renamed
    # out from under it. The app's own backend opens it directly rather than
    # through this gate, so writes keep working.
    "md-notebook-staging",
