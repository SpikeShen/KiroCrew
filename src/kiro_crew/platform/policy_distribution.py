...
def _recompose_differs(central: GovernanceCeiling) -> bool:
    """Would re-folding the ladder around *central* change what is installed?
    ...
    is the whole ladder's content, not identity. No second digest is kept: the
    installed ceiling itself is the record of what the last fold produced.
    """
    from kiro_crew.platform.context import current_context
    from kiro_crew.platform.governance import compose_installed_ceiling

    return compose_installed_ceiling(central) != current_context().governance
