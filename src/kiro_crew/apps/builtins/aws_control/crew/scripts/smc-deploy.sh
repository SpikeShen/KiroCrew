...
smc_main() {
  ...
  mkdir -p "$WORK"
  # 700, because _curl_signed writes a credential in here. Best-effort: a
  ...
  # A RESUME skips step 0, and step 0 is where the credentials were asked who they are.
  # So `--from 2` used the account recorded by an earlier run and never checked that the
  # profile still points there: repoint it between runs -- a second `aws sso login`, an
  # edited profile, a changed AWS_PROFILE -- and CloudFormation mutates a DIFFERENT
  # account than the one the operator confirmed, silently, because every later step reads
  # this file rather than asking.
  #
  # Re-confirming the IDENTITY is not the same as re-asking the operator. They already
  # said yes to this account; the question a resume has to answer is whether the
  # credentials still name it. Same rule the Crews pane states as a 409 account_mismatch,
  # which is a disclosure refusal rather than an error, and it belongs here for the same
  # reason: acting on the wrong account is worse than stopping.
  if [ "$FROM" -gt 0 ]; then
    local now_account=""
    now_account="$(aws_ sts get-caller-identity --query Account --output text)" \
      || die "cannot reach AWS with profile '$PROFILE' to confirm the account. Run: aws sso login --profile $PROFILE"
    if [ "$now_account" != "$ACCOUNT" ]; then
      die "this resume would act on the WRONG ACCOUNT.
   step 0 recorded  $ACCOUNT
   the profile now signs in to  $now_account
   Nothing has been changed. Either point profile '$PROFILE' back at $ACCOUNT, or drop
   --from and run from step 0 so the account is confirmed for this deploy."
    fi
    ok "account $ACCOUNT still confirmed for this resume"
  fi
  _ev_account "$ACCOUNT"
  ...
  if [ -n "${KIRO_API_KEY:-}" ]; then
    ...
    # `unreadable` is NOT "unchanged". secret_digest returns it when the secret exists
    # but its value could not be read, which is exactly the case a write-capable role
    # without GetSecretValue produces. The prior value is unknown, so the only safe
    # reading is that it changed: a task reads its secret once at start, so skipping the
    # restart leaves the crew serving the old credential while the deploy says success.
    #
    # Empty stays unchanged, and that is correct rather than an oversight: empty means
    # the secret did not exist, so this run CREATED it and there is no older task to
    # restart.
    if [ "$before" = "unreadable" ]; then
      SECRET_ROTATED=1
      note "            could not read the previous value, so treating this as a rotation"
...
secret_digest() { # NAME -> 12 hex chars, or empty for absent, or 'unreadable'
  if [ "$DRY_RUN" -eq 1 ]; then
  ...
  # THREE outcomes, not two. An earlier version returned empty for every failure and
  # said in its own comment that "could not read it has to count as changed" -- but the
  # caller spelled that as `[ -n "$before" ] && ...`, so an unreadable value counted as
  # UNCHANGED and the forced restart was skipped. A task reads its secret once at start,
  # so the crew then kept serving with the old credential and the deploy reported success.
  #
  # describe-secret is the discriminator, and it is the right one precisely because it is
  # a DIFFERENT permission from get-secret-value: a role that can write the secret but
  # not read it back -- the case that produced this bug -- answers here and fails above.
  if [ -n "$(secret_arn "$1")" ]; then
    printf 'unreadable'
  fi
  return 0
}
...
sig() { # PATH -> http code
  ...
  _curl_signed -o "$WORK/resp" -w '%{http_code}' "${ControlBaseUrl}$1"
}

# EVERY credential goes in a MODE 600 FILE that is deleted when the call returns.
#
...
#
# Two gaps this closes, both found after the --user fix and both real:
#
#   1. The SESSION TOKEN was still passed as `-H x-amz-security-token: ...` on the
#      command line at all three call sites. Moving the long-lived key out of argv
#      while leaving the token in it protects the wrong half: the token is what is
#      currently valid.
#   2. The file was created and NEVER REMOVED, so it sat in $WORK for the rest of the
#      run holding usable credentials. Mode 600 keeps other users out; it does nothing
#      about anything running as this user, which on a deploy box is the exposure that
#      matters.
#
# So the file is built, used and removed inside ONE function, with the removal on a
# RETURN trap: curl failing under `set -e`, or any early return, still takes the
# credential file with it.
#
# Truncated via a subshell umask, so it is never briefly world-readable.
_curl_signed() { # CURL_ARGS... -> curl's exit status
  local f="$WORK/.curlrc-auth"
  trap 'rm -f "$f"' RETURN
  ( umask 077; : > "$f" )
  # An `if` rather than `[ -n ... ] && printf`: the && form returns non-zero when the
  # token is unset, which under `set -euo pipefail` aborts the deploy. That is exactly
  # how secret_digest broke the first real deploy earlier in this branch.
  {
  ...
  curl -sS --aws-sigv4 "aws:amz:$REGION:execute-api" -K "$f" "$@"
}
