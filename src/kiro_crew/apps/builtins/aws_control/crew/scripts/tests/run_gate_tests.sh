...
# --- no credential reaches a command line, and none outlives the call ------------
# Two gaps found AFTER the original --user fix, both real:
#   1. the session token was still passed as `-H x-amz-security-token: ...` on argv at
#      all three call sites, so the half that is currently valid was the exposed half;
#   2. the config file was created and never removed, leaving usable credentials in
#      $WORK for the rest of the run. Mode 600 stops other users, not anything running
#      as this user, which on a deploy box is the exposure that matters.
# Measured with a curl stand-in that records its own argv: token, key and secret all 0,
# and the file gone on return. An earlier /proc probe came back INCONCLUSIVE because
# curl exited first, which is why the shim exists.
_CURLFN='_curl_signed'
grep -q "^${_CURLFN}() {" "$SCRIPT"
assert_ok "curl auth: every signed call goes through one wrapper" $?
assert_eq "curl auth: no session token on any command line" \
  "0" "$(grep -cE '\-H "x-amz-security-token' "$SCRIPT" || true)"
assert_eq "curl auth: the old file-returning helper is gone" \
  "0" "$(grep -c '_curl_auth_file()' "$SCRIPT" || true)"
grep -qE "trap 'rm -f \"\\\$f\"' RETURN" "$SCRIPT"
assert_ok "curl auth: the credential file is removed on every return path" $?
# Every curl that signs must be the wrapper's own line, so a new call site cannot
# quietly reintroduce argv credentials.
assert_eq "curl auth: exactly one curl invocation signs" \
  "1" "$(grep -cE 'curl .*--aws-sigv4' "$SCRIPT" || true)"
# The unset-token branch must be an `if`, not `[ -n ] &&`: the && form returns non-zero
# when the token is absent, and under `set -euo pipefail` that aborts the deploy. That
# is how secret_digest broke the first real deploy in this branch.
grep -q 'if \[ -n "${AWS_SESSION_TOKEN:-}" \]; then' "$SCRIPT"
assert_ok "curl auth: the absent-token branch cannot abort the run" $?
...
# Was three `-K "$(_curl_auth_file)"` call sites. They now share ONE wrapper, so the
# property moved: three callers reach curl only through it, and it is the single place
# that names the config file.
assert_eq "curl auth: all three signed calls go through the wrapper" \
  "3" "$(grep -cE '^ *_curl_signed( |$|\\)' "$SCRIPT" || true)"
[ "$(grep -c -- '-K "$f"' "$SCRIPT")" -eq 1 ]
assert_ok "curl auth: exactly one place names the config file" $?
...
# --- an UNREADABLE prior secret is a rotation, not "unchanged" --------------------
# `before=""` used to mean both "the secret does not exist" and "I could not read it",
# and the caller's `[ -n "$before" ] && ...` turned the second into UNCHANGED. A task
# reads its secret once at start, so the forced restart was skipped and the crew kept
# serving the OLD credential while the deploy reported success. The role that produces
# this can write the secret but not read it back, which is why describe-secret is the
# discriminator: it is a DIFFERENT permission from get-secret-value.
#
# Driven for real, not grepped: `aws_` is stubbed so get-secret-value fails and
# describe-secret answers, which is exactly that role.
_saved_dry="${DRY_RUN:-}"
DRY_RUN=0
aws_() {
  case "$*" in
    *"get-secret-value"*) return 254 ;;                       # AccessDeniedException
    *"describe-secret"*)  printf 'arn:aws:secretsmanager:us-west-2:1:secret:k'; return 0 ;;
    *) return 1 ;;
  esac
}
assert_eq "secret_digest: exists but unreadable -> 'unreadable'" \
  "unreadable" "$(secret_digest smc-key)"
aws_() {
  case "$*" in
    *"get-secret-value"*) return 254 ;;
    *"describe-secret"*)  return 254 ;;                       # absent: neither answers
    *) return 1 ;;
  esac
}
assert_eq "secret_digest: absent -> empty, so creation is not a rotation" \
  "" "$(secret_digest smc-key)"
aws_() {
  case "$*" in
    *"get-secret-value"*) printf 'the-stored-key'; return 0 ;;
    *) return 1 ;;
  esac
}
assert_eq "secret_digest: readable -> the digest of the stored value" \
  "$(printf 'the-stored-key' | sha256sum | cut -c1-12)" "$(secret_digest smc-key)"
unset -f aws_
if [ -n "$_saved_dry" ]; then DRY_RUN="$_saved_dry"; else unset DRY_RUN; fi
# The caller has to ACT on it. Asserted on shape because the branch sits inside the
# step-5 flow rather than a function that can be called on its own.
#
# The sed output is captured FIRST rather than piped into `grep -q`. Piped, grep exits on
# the first match and sed dies of SIGPIPE, which pipefail surfaces as 141 -- so the
# assertion reported rc=141 while the property it checks was true. Same family as
# `cmd | tail` handing $? to tail.
_step5="$(sed -n '/before="\$(secret_digest/,/^    fi$/p' "$SCRIPT")"
case "$_step5" in
  *'if [ "$before" = "unreadable" ]; then'*) true ;;
  *) false ;;
esac
assert_ok "api key: an unreadable prior value forces the restart" $?
case "$_step5" in *'SECRET_ROTATED=1'*) true ;; *) false ;; esac
assert_ok "api key: that branch is the one that sets the rotation flag" $?
unset _step5

# --- the base tag is allowed to move, and ONLY that tag -------------------------
# build_image.sh calls `smc-base-<ARCH>` a MOVING tag. The repository declared every tag
# IMMUTABLE, so from the SECOND base publication the move was refused -- and the driver
# resolving that tag afterwards does not fail, it returns the PREVIOUS digest. A wrong
# answer, so every later deploy ran a stale base image and every run reported success.
BASE_TPL="$DEPLOY_DIR/../templates/base.yaml"
grep -q 'ImageTagMutability: IMMUTABLE_WITH_EXCLUSION' "$BASE_TPL"
assert_ok "ecr: immutability carries an exclusion" $?
grep -q "ImageTagMutabilityExclusionFilterValue: 'smc-base-\*'" "$BASE_TPL"
assert_ok "ecr: the exclusion names the moving base tag" $?
grep -q 'ImageTagMutabilityExclusionFilterType: WILDCARD' "$BASE_TPL"
assert_ok "ecr: the filter type is the documented WILDCARD" $?
# The exclusion must not widen to crew images, whose tags start `smc-<arch>-`.
case "smc-X86_64-abcdef123456-0011223344ff-999" in
  smc-base-*) false ;; *) true ;;
esac
assert_ok "ecr: the exclusion does NOT match a crew image tag" $?
case "smc-base-X86_64" in smc-base-*) true ;; *) false ;; esac
assert_ok "ecr: the exclusion DOES match the base tag" $?
# A refused move must not pass as a warning: the OUTCOME is verified.
grep -q 'MOVED_TO' "$BUILDER"
assert_ok "base tag: the move is verified by where the tag points" $?
# Captured, not piped: see the SIGPIPE note above. This one happened to pass on timing,
# which is worse than not having it.
_movecheck="$(sed -n '/MOVED_TO=/,/^  fi$/p' "$BUILDER")"
case "$_movecheck" in *'die "the '"'"'smc-base-'*) true ;; *) false ;; esac
assert_ok "base tag: a tag pointing elsewhere is fatal" $?
case "$_movecheck" in *'wanted ${REPO_DIGEST}'*) true ;; *) false ;; esac
assert_ok "base tag: the refusal names the digest to deploy with instead" $?
unset _movecheck

# --- the crew image tag names the wheel's CONTENT ------------------------------
# WHEEL_COMMIT is a commit id, so an uncommitted change in --kirocrew-src left the tag
# identical while the wheel bytes differed -- and an existing tag is REUSED here, so the
# deployment referenced wheel bytes nobody asked for. CREW_ROOT already had a `-dirty`
# marker; the other source tree did not. WHEEL_SHA256 was already computed for the labels.
grep -q 'PUSH_TAG="${IMAGE_REPO}:smc-${CFN_ARCH}-${WHEEL_COMMIT:0:12}-${WHEEL_SHA256:0:12}-${APP_REV}"' "$BUILDER"
assert_ok "crew image tag: names arch, wheel commit, wheel sha256 and app revision" $?
# Two wheels at the SAME commit must not collide, and two identical wheels must still
# share a tag or reuse stops working.
_tag() { printf 'smc-%s-%s-%s-%s' "$1" "${2:0:12}" "${3:0:12}" "$4"; }
[ "$(_tag X86_64 aaaaaaaaaaaabbbb 1111111111112222 rev1)" \
  != "$(_tag X86_64 aaaaaaaaaaaabbbb 3333333333334444 rev1)" ]
assert_ok "crew image tag: same commit, different wheel bytes -> different tags" $?
assert_eq "crew image tag: identical inputs -> identical tag, so reuse still works" \
  "$(_tag X86_64 aaaaaaaaaaaabbbb 1111111111112222 rev1)" \
  "$(_tag X86_64 aaaaaaaaaaaabbbb 1111111111112222 rev1)"

# --- a resume re-confirms WHICH ACCOUNT the credentials name ---------------------
# Step 0 is where `sts get-caller-identity` runs and where the operator says yes. A
# resume skips step 0, so `--from 2` used the account recorded by an earlier run and never
# checked that the profile still points there. Repoint it in between -- another
# `aws sso login`, an edited profile, a changed AWS_PROFILE -- and CloudFormation mutates
# a DIFFERENT account than the one that was confirmed, with no line of output saying so.
#
# Driven end to end rather than grepped: SMC_FIX_ACCOUNT is what the dry-run fixture
# answers `get-caller-identity` with, so the repointed profile is a real input here.
#
# The two ids live in variables because the scrub gate refuses `account.?[0-9]{12}` --
# twelve digits sitting directly after the word account is what a real leaked account id
# looks like. The driver's own fixture default passes only because `:-` separates them.
_recorded_id=111122223333
_repointed_id=999988887777
_RW="$(mktemp -d)"
KIRO_API_KEY='aws-kiro-abcdefghijklmnopqrstuvwxyz0123456789' \
  SMC_WORK="$_RW" SMC_FIX_ACCOUNT="$_recorded_id" \
  bash "$SCRIPT" --dry-run --crew acme-support --memory-mode chatbot >/dev/null 2>&1
assert_eq "resume: step 0 records the confirmed account" \
  "$_recorded_id" "$(cat "$_RW/account" 2>/dev/null)"

out="$(KIRO_API_KEY='aws-kiro-abcdefghijklmnopqrstuvwxyz0123456789' \
  SMC_WORK="$_RW" SMC_FIX_ACCOUNT="$_recorded_id" \
  bash "$SCRIPT" --dry-run --crew acme-support --memory-mode chatbot --from 2 2>&1)"
assert_contains "resume: the same account is confirmed and the resume proceeds" \
  "$out" "still confirmed for this resume"

out="$(KIRO_API_KEY='aws-kiro-abcdefghijklmnopqrstuvwxyz0123456789' \
  SMC_WORK="$_RW" SMC_FIX_ACCOUNT="$_repointed_id" \
  bash "$SCRIPT" --dry-run --crew acme-support --memory-mode chatbot --from 2 2>&1)"
assert_contains "resume: a repointed profile is REFUSED, not acted on" \
  "$out" "WRONG ACCOUNT"
assert_contains "resume: the refusal names the recorded account" "$out" "$_recorded_id"
assert_contains "resume: the refusal names the account now signed in to" "$out" "$_repointed_id"
assert_contains "resume: the refusal says nothing was changed" "$out" "Nothing has been changed"
assert_not_contains "resume: a refused resume creates nothing" "$out" "changeset"
rm -rf "$_RW"
unset _recorded_id _repointed_id
