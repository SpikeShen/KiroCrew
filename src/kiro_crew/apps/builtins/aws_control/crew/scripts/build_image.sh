...
# A git repo, not a `.git` DIRECTORY. In a linked worktree `.git` is a FILE holding a
# gitdir pointer, so `[ -d ... ]` refused every worktree checkout -- and refused it HERE,
# at step 4, after the driver had already created the base stack at step 2. The account was
# left half-deployed by a check that a normal way of working cannot pass.
#
# smc-deploy.sh already knew: `resolve_kirocrew_src` uses `-e` and its comment says
# "Validate it here so a wrong path is caught by the driver at preflight, not by
# build_image.sh at step 4". Two spellings of one rule, and this copy drifted.
#
# `rev-parse --is-inside-work-tree` beats both: `-e` only proves something named `.git`
# exists, while this proves git will actually operate on the path, which is what the wheel
# build below needs.
git -C "${KIROCREW_SRC}" rev-parse --is-inside-work-tree >/dev/null 2>&1 \
  || die "Kiro Crew source checkout not a git repo: ${KIROCREW_SRC}"
...
if [ -n "${IMAGE_REPO}" ]; then
  ...
  #
  # The WHEEL'S CONTENT, not just the commit it came from. WHEEL_COMMIT is a commit id,
  # and an uncommitted change in --kirocrew-src leaves it unchanged while the wheel bytes
  # differ -- line 142 only WARNS about that. The tag was therefore identical for two
  # different wheels, the existing tag was reused, and the deployment ran wheel bytes
  # nobody asked for. Note the asymmetry this closes: CREW_ROOT already got a `-dirty`
  # marker above, so one of the two source trees was handled and the other was not.
  # WHEEL_SHA256 is already computed for the labels and the manifest, so it costs
  # nothing here and is strictly better than a dirty timestamp: it is content-addressed,
  # so two identical wheels still share a tag and reuse still works.
  PUSH_TAG="${IMAGE_REPO}:smc-${CFN_ARCH}-${WHEEL_COMMIT:0:12}-${WHEEL_SHA256:0:12}-${APP_REV}"
  ...
  # rebuilding.
  #
  # This used to add "and it is the operation an immutable repository still permits on a
  # tag that is meant to move". That was a false claim about ECR: immutability refuses to
  # repoint an existing tag whatever mechanism asks, so from the SECOND base publication
  # onward the move was refused. base.yaml now excludes `smc-base-*` from immutability,
  # which is what makes the move legal at all.
  #
  # The OUTCOME is checked rather than the exit code, because a warning was not enough.
  # What the driver does later is resolve this tag to a digest, and a refused move does
  # not make that resolution fail -- it makes it return the PREVIOUS digest. A wrong
  # answer, not an error: every later deploy would run a stale base image and every run
  # would report success. So the tag must actually point at this run's digest before this
  # step is allowed to pass.
  ...
  docker buildx imagetools create --tag "${IMAGE_REPO}:smc-base-${CFN_ARCH}" "${IMAGE_REPO}@${REPO_DIGEST}" \
    || echo "WARNING: 'imagetools create' failed for the 'smc-base-${CFN_ARCH}' tag; verifying where the tag points" >&2
  MOVED_TO="$(docker buildx imagetools inspect "${IMAGE_REPO}:smc-base-${CFN_ARCH}" \
    --format '{{json .Manifest.Digest}}' 2>/dev/null | tr -d '"' || true)"
  if [ "${MOVED_TO}" != "${REPO_DIGEST}" ]; then
    die "the 'smc-base-${CFN_ARCH}' tag does not point at this build.
   wanted ${REPO_DIGEST}
   found  ${MOVED_TO:-<the tag does not resolve>}
   The image itself IS pushed, so nothing is lost: deploy with
     --base ${IMAGE_REPO}@${REPO_DIGEST}
   Left as a warning this became a stale base image on every later deploy, because
   resolving the tag then returns the PREVIOUS digest rather than failing."
  fi
else
