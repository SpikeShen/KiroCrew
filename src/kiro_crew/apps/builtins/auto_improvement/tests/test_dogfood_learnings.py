...
#: from `git fetch --porcelain` STDOUT instead of reading the mutable `FETCH_HEAD` ref (#8452):
    ...
    def test_a_non_fast_forward_triggers_exactly_one_rebase_and_retry(
        ...
        def _fake_git(a, cwd):
            ...
            if porcelain is not None:
                return porcelain
            # The retry now resolves the REPLAYED commit to a full object id and pushes THAT
            # rather than `HEAD` (#8452), so the double has to answer that question; an empty
            ...
            # commit (#8452): an equivalent upstream patch makes git drop ours, and an
        ...
        assert ["fetch", "--porcelain", "https://x/y.git", "b"] in gits, (
            ...
            "instead of the mutable FETCH_HEAD ref (#8452)"
        )
        # BOTH ends of the rebase are object ids. `rebase FETCH_HEAD` with no revision replays
        # whatever the BRANCH points at, and that bare form no longer exists (`src` is
        # required); naming FETCH_HEAD as the BASE was its own defect, since that ref is a
        # mutable file the reviewer can repoint at an unscanned commit (#8452).
        assert [
            "rebase",
            FETCHED_BASE,
        ...
        # post-push `rev-parse HEAD` would name whatever HEAD points at by then (#8452).
...
class TestARebasedTreeIsReVerifiedBeforePublishing:
    ...
    def test_the_replayed_tree_is_re_verified_before_the_second_push(
        ...
        def _fake_git(a, cwd):
            ...
            # id and pushes THAT, so the double must answer `rev-list` (#8452).
            # `rev-list --count FETCH_HEAD..HEAD` proves the rebase replayed exactly ONE
            # commit (#8452): an equivalent upstream patch makes git drop ours, and an
            # unanswered count is correctly read as "not one" and refuses.
            if a[:2] == ["rev-list", "--count"]:
    ...
    def test_the_ledger_records_the_sha_that_actually_landed(self) -> None:
        ...
        # described callers that no longer exist once `src` became required.
        assert (
...
class TestOnlyTheCommitThePipelineMadeIsPublished:
    """#8452 mitigation 2: do not push what a worker did not commit.
    ...
    def test_the_report_only_reviewer_has_no_edit_grant(self) -> None:
        ...
        HEAD. Raised by the first-principles review of #8452."""
        import inspect
    ...
    def _driver(clone: Path, reviewer: object):  # type: ignore[no-untyped-def]
        ...
        def _publish(  # type: ignore[no-untyped-def]
            ...
            # whole subject of #8452, so a spy that dropped it could not tell publishing the
    ...
    def test_a_move_after_the_scan_cannot_change_what_is_published(self, tmp_path: Path) -> None:
        ...
        THEN do the credential scan and the push run. Both used to re-read ``HEAD``, so a
        background process the reviewer left behind could move it in between -- the scan would
        read one commit and the push would publish another, neither of them verified.
