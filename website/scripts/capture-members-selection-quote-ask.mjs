 ...
 *                   Ask in Side Chat and Copy (before: Copy only)
...
async function selectReply(page) {
  ...
  await page.getByRole('button', { name: 'Ask in Side Chat' }).waitFor({ state: 'visible', timeout: 5_000 })
...
// 01 — the toolbar over a member reply: Quote / Ask in Side Chat / Copy.
{
  ...
  const toolbar = page.getByRole('button', { name: 'Ask in Side Chat', exact: true }).locator('xpath=ancestor::div[contains(@class,"z-[9999]")]')
  const labels = await toolbar.getByRole('button').allTextContents()
  check('01-toolbar three actions', labels.map(s => s.trim()).join(' / ') === 'Quote / Ask in Side Chat / Copy', `buttons=${labels.map(s => s.trim()).join(' / ')}`)
