...
async function selectReplyB(page) {
  ...
  await page.getByRole('button', { name: 'Ask in Side Chat', exact: true }).waitFor({ state: 'visible', timeout: 5000 })
...
async function main() {
  ...
  // ---- 1. toolbar in the NON-active pane ----
  {
    ...
    check('1-split-toolbar three actions', labels.join(' / ') === 'Quote / Ask in Side Chat / Copy', `buttons=${labels.join(' / ')}`)
