...
| Transcript from agent replay (prototype, off by default) | A resumed session's transcript rebuilt from the agent backend's own resume replay — thinking and tool detail the log omits — with a "rebuilt from the agent's record" banner; the log still supplies the rows the agent never saw | Settings → Chat → **Show the agent's own record when a session resumes**, then reopen a session after a gateway restart | `pages/ChatPage.tsx` (banner), `store/chatSlice.ts` (`slotTranscriptSource`) | `chat_replay.py`, `chat_handlers.py` (`api_chat_slot_detail`), `handlers/files.py` (`replay_from_acp` toggle) | `GET /api/chat/slots/{slot}` (`transcript_source`, `replay_report`), `GET,PUT /api/dashboard/config` (`replay_from_acp`) |
...
| `chat` | Chat behavior preferences, plain vs highlighted diffs, and the **Show the agent's own record when a session resumes** prototype toggle (`dashboard.replay_from_acp`) | `ChatPanel.tsx` | `handlers/core.py`, `handlers/files.py` | `GET,PUT,PATCH /api/config/kirocrew`, `GET,PUT /api/dashboard/config` |
