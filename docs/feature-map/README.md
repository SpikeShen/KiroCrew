...
| Crew Members | One durable pinned DM thread per crew member, read as a conversation (a chat-profile projection that hides tool/process rows behind a disclosure) with the member's escalations rendered as actionable cards — countdown, default action and selectable option replies (click to choose, then Send); the detail drawer lists the worker sessions the member is driving (live `slots` frames filtered on `created_by`) | `/members` — rail row when the crew preview is on | `pages/members/MembersPage.tsx`, `pages/members/chatProjection.ts`, `pages/members/EscalationCard.tsx`, `pages/members/WorkingIndicator.tsx`, `pages/members/ProcessDisclosure.tsx`, `pages/members/ChatFoldRow.tsx`, `pages/members/chatProfileRenderers.tsx`, `pages/members/escalationState.ts`, `pages/members/useEscalationIndex.ts` | `handlers/members.py`, `slot_projection.py` (`created_by`), `session_control.py` (`escalate_to_user`, the human as a peer), `src/kiro_crew/crew_conversation.py` (the per-member conversation index) | `GET /api/members`, `POST /api/members/{slug}/thread`, `GET /api/members/{slug}/activity`, `GET,PUT /api/members/{slug}/rules`, `GET /api/members/{slug}/conversation`, `GET /api/ws` (`slots`), `POST /api/session-control/escalate` |
