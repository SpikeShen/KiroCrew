...
| Voice reply and dictation | TTS on replies; streaming mic transcription | Composer mic; Settings → Voice | `components/ChatInput.tsx` | `chat_voice.py`, `stt_stream.py` | `POST /api/voice/synthesize`, `GET /api/voice/system-voices`, `GET /api/ws/stt`, `POST /api/stt/transcribe` |
