...
|---|---|---|---|---|---|
| Crews | Named agent bindings — which agent, model and workspace; per-crew custom avatar (hand-picked ghost traits or an uploaded picture, edited in the crew editor's avatar builder; a crew record can also name an appearance pack, but the picker for it is not built yet — see Appearance library) | `/capabilities?tab=crews` | `pages/KiroCrewAgentsPage.tsx`, `components/CrewAvatarBuilder.tsx`, `components/CrewAvatar.tsx` | `handlers/agents.py` | `GET /api/agent/config`, `GET,PUT /api/config/default-agent`, `POST /api/agents`, `PUT,DELETE /api/agents/{name}`, `GET,POST /api/agents/{name}/avatar` |
| Crew appearance library | BACKEND ONLY so far — no user-reachable UI in this release. The dashboard's own library of appearance packs (art plus optional per-state sound cues) a crew can wear, rooted at `<data home>/appearance-library/`. Separate from Crew Companion's library: that app is independent and keeps its own packs; the two share only the pack format, so a pack exported from the Companion gallery imports here unchanged. A crew record can name a pack (`avatar: {"kind":"pack","id":...}`) and the API can list, read, import, PetDex-fetch and delete packs; deleting a pack a crew wears is refused by name unless forced. The crew-editor picker that lets a user CHOOSE a pack is a separate frontend change and does not exist yet | Not reachable from the dashboard UI yet — API only | None yet (`components/CrewAvatarBuilder.tsx` gains the picker in a later change) | `handlers/appearances.py`, `dashboard/appearances.py` (crew store), `apps/builtins/crew_companion/appearances.py` (shared store class) | `GET /api/appearances`, `GET /api/appearances/{id}`, `GET /api/appearances/{id}/slot/{slot}`, `GET /api/appearances/{id}/sound/{state}`, `POST /api/appearances/import`, `POST /api/appearances/petdex/fetch`, `DELETE /api/appearances/{id}` |
