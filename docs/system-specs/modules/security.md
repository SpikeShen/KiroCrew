...
(†) `SSH_AUTH_SOCK` is kept, not scrubbed, when `agent.sandbox_forward_ssh_auth_sock` is on — see the opt-in paragraph below.
...
**`SSH_AUTH_SOCK` forward opt-in (`agent.sandbox_forward_ssh_auth_sock`, default OFF, issue #8104)** — with a passphrase-protected SSH signing key, git commit signing inside the sandbox fails because the only path to the key is the operator's ssh-agent, reached through `SSH_AUTH_SOCK`, which the scrub above removes. When the operator sets this flag, the single `SSH_AUTH_SOCK` key is kept in the agent subprocess environment; every other credential prefix is still scrubbed, so the opt-in cannot widen into a general env passthrough. The property that makes this acceptable is that **the socket grants USE of the agent's keys, not possession**: the private key material is never forwarded, and under the **strict** tier `~/.ssh` is hidden (Linux bind-mount) / read-denied (macOS Seatbelt `(deny file-read* ~/.ssh)`, exempting only `known_hosts`), so the agent still cannot read the key files even with the socket. The trade-off is real and documented on the config field: any code the agent runs can authenticate as the operator through the socket for the session's lifetime, not commit signing alone — which is why it is off by default and operator-declared. One filter (`sandbox._agent_scrub_prefixes`) applies the opt-in at the two OS-launcher scrub sites (the Linux namespace launcher and the macOS Seatbelt `env -u` renderer); the parent-side agent scrub applies it in `scrub_agent_subprocess_env` (the ACP enforcement point) rather than in the generic `scrub_env`, which also serves non-agent callers (tailscale host children, `sandboxed_spawn_argv` spawns) that must keep the socket scrubbed. The forward decision is a boolean resolved ONCE off the event loop on the agent spawn path (`_forward_ssh_auth_sock`, called inside the existing off-loop environment-prep hop in `AcpClient._spawn` / `AcpRuntime.spawn`) and threaded down as an explicit parameter -- exactly like `strip_python_env` -- so no synchronous config read runs on the asyncio event loop, and the generic launcher builders default the flag off. The forward's blast radius is therefore exactly the agent child: a third-party app `openCommand` or `sandboxed_spawn_argv` spawn going through the same generic launcher never re-admits the socket. The shared `_SENSITIVE_ENV_PREFIXES` constant is **never** mutated by the opt-in, so MCP declared-env forwarding (`manager.is_credential_env_key`, below) keeps refusing `SSH_AUTH_SOCK` regardless of this flag.
...
- **A relative traversal root resolves against the command's own `cd`, never the gateway's cwd** (`_is_cwd_relative`, `_alt_root_reaching_fence`, `_alt_implicit_cwd_root`, `_alt_substitution_assignment_fences`, `_find_anchor_relative_roots`, `_find_traversal_reaches_fence`): the two traversal passes used to resolve a root such as `.` through `path_contains_sensitive` / `_candidate_forms` against THIS PROCESS's working directory, and only afterwards join it onto a `cd` base from the same line. The agent's shell never runs in the gateway's directory, and the desktop app's gateway runs from `/`, an ancestor of every fenced store, so every `grep -r X .`, `rg X .`, `fd .env -x cat` and `find . -type f -exec cat {} +` was refused whatever `cd` preceded it, while the same command on a gateway launched from a checkout was allowed: the verdict depended on where the gateway happened to start. Now a root spelled relative to the shell's directory (not absolute, not `~`-anchored, not carrying a still-unexpanded `$`/`%` variable) is joined onto every `cd` base the line entered and judged on those real paths, and with no `cd` on the line it is UNKNOWABLE from the text and left to the sandbox, the stance `_find_root_readings` already takes for a root held in an unread variable. The same test applies to the stage tokens `_alt_substitution_assignment_fences` reads when a variable's value is a computed substitution: a relative token there (`D=$(ls .)`) is skipped rather than resolved against the process cwd. In the `find` pass the unanchored root is kept as written so the width and brace budgets still refuse on the text's shape, and only the fence comparison skips it. The same rule closes a residual the alt pass's block comment used to name: a traversal with NO root at all after a `cd` into the fence (a `cd` into the crew data home followed by a bare `rg secret`) is now denied, because the implicit root is the `cd` base rather than the gateway's directory. **Stated residual**: a relative root with no `cd` on the line is judged as unknowable even when the shell's real cwd IS the home directory (a session whose project directory is `~`); the literal, normalized and `cd`-tainted spellings of every fenced path are still refused by passes 0 to 2, and every anchored root is unchanged. Pinned in both directions by `test/test_security_relative_traversal_roots.py`, whose allow cases run with the process cwd pinned to a directory that holds the per-test crew data home (the shape of the `/` gateway) so they fail on the old resolution, and whose deny cases keep every knowable-root catch.
