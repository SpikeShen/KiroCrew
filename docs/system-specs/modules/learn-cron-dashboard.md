...
`avatar` field (see `config.md`, *Per-crew avatar override*) pins one of two
...
`invoked` audit (503, mutation not run) — a member relaxing the ceiling must leave a SEL record. The arm path's trust
