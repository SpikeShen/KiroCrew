...
| [rfc-acp-replay-as-transcript-source.md](rfc-acp-replay-as-transcript-source.md) | `in-progress` | P0 ships with [#8862](https://github.com/kirodotdev/KiroCrew/pull/8862): `dashboard.replay_from_acp` (default off) makes `AcpRuntime.load_session` retain the `session/load` replay frames and `dashboard/chat_replay.py` render a resumed transcript from them with the JSONL as sidecar. Nothing on main; the JSONL is still written in full and P1–P3 (messageId anchor, stop writing the body, OAuth to live state) are unstarted |
