...
| [rfc-central-governance-ceiling.md](rfc-central-governance-ceiling.md) | `in-progress` | Steps 1-5 are on this branch: the MDM-managed tier, its root-owned trust check, central-above-env precedence, tighten-only subordinates, and Ed25519 policy signing; the dated `break_glass` override (step 6) was withdrawn before merge and is a follow-up (#9106). Step 7 (server-side attestation, the only step that binds local root) is a documented non-goal |
